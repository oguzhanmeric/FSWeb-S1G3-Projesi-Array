...
function kopyala(copyTat) {
  let x = [...copyTat]
  return x;
} 
console.log("kopyalama func : ", kopyala(orijinalTatlar));
...
console.log("Görev 6 - ismeGoreCesitCikar :", ismeGoreCesitCikar(orijinalTatlar, "Tarçın"));
/* Görev 7:
...
let dizin = orijinalTatlar;
let str = dizin.join(" "); //ilk önce bütün dizideki elemanları arasında boşluk olacak şekilde birleştir
let words = str.split(" "); // tekrar yeni bir array olacak şekilde ayır
let numWords = words.length;
let ort = numWords / dizin.length;    // function yapmaya calıstıgımda hep hata aldım. 
console.log("ortalama kelime sayısı : " + ort);
...
function rastgeleTatlar(orijinalTatlar, yeniTatlar1, mevsimlikTatlar, bolgeselTatlar){

  const tumTatlar = orijinalTatlar.concat(yeniTatlar1,mevsimlikTatlar,bolgeselTatlar);
  let secilenler=[];
  while (secilenler.length<25) {
    const rastgeleIndex = Math.floor(Math.random()*tumTatlar.length);
    if (!secilenler.includes(tumTatlar[rastgeleIndex])) {
      secilenler.push(tumTatlar[rastgeleIndex]);
    }
  }
  /*kod buraya*/
} 
...
console.log(rastgeleTatlar(orijinalTatlar, yeniTatlar1, mevsimlikTatlar, bolgeselTatlar));
